...
chart. GitHub's markdown renderer strips `<svg>`/`<style>` tags from README
content entirely (there's no way around this - it's not a formatting issue,
the tags simply aren't on GitHub's sanitizer allowlist), so a hoverable SVG
can't be embedded inline here. Download
[`claims_heatmap_interactive.svg`](examples/claims_heatmap_interactive.svg)
and open it directly in a browser to hover a day and see its claim count.*
